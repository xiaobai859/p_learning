...
(function (window, undefined) {
  ...
  function _inString(val, str, delimiter) {
    ...
    return (delimiter + str + delimiter).indexOf(delimiter + val + delimiter)
        >= 0;
  }
  ...
  _extend(KEvent, {
    init: function (el, event) {
      ...
      if (self.pageX == null && self.clientX != null) {
        ...
        self.pageX = self.clientX + (d && d.scrollLeft || body
            && body.scrollLeft || 0) - (d && d.clientLeft || body
            && body.clientLeft || 0);
        self.pageY = self.clientY + (d && d.scrollTop || body && body.scrollTop
            || 0) - (d && d.clientTop || body && body.clientTop || 0);
      }
      if (!self.which && ((self.charCode || self.charCode === 0) ? self.charCode
          : self.keyCode)) {
      ...
      if (!self.which && self.button !== undefined) {
        self.which = (self.button & 1 ? 1 : (self.button & 2 ? 3 : (self.button
        & 4 ? 2 : 0)));
      }
      switch (self.which) {
        case 186 :
      ...
      tag = tag.replace(/(\s+class=["']?)([^"']*)(["']?[\s>])/,
          function ($0, $1, $2, $3) {
            if ((' ' + $2 + ' ').indexOf(' ' + className + ' ') < 0) {
              return $2 === '' ? $1 + className + $3 : $1 + $2 + ' ' + className
                  + $3;
            } else {
              return $0;
            }
  ...
  function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
    ...
    html = html.replace(re, function ($0, $1, $2, $3, $4, $5, $6) {
      ...
      if (_BLOCK_TAG_MAP[tagName] && !_PRE_TAG_MAP[tagName]) {
        if (wellFormatted) {
          if (startSlash && tagStack.length > 0 && tagStack[tagStack.length - 1]
              === tagName) {
            tagStack.pop();
          } else {
          ...
          endNewline = '\n';
          for (var i = 0,
              len = startSlash ? tagStack.length : tagStack.length - 1; i < len;
              i++) {
    ...
    function byClass(className, tag, root) {
      ...
      if (root.getElementsByClassName) {
        ...
        els = doc.querySelectorAll(
            (root.nodeName !== '#document' ? root.nodeName + ' ' : '') + tag
            + '.' + className);
        for (i = 0, len = els.length; i < len; i++) {
          el = els[i];
        ...
        if (start > 0) {
          if (node.nodeType == 1) {
            ...
            if (node == copyRange.startContainer) {
              textNode = splitTextNode(node, copyRange.startOffset,
  ...
  _extend(KRange, {
    ...
    createBookmark: function (serialize) {
      ...
      if (endNode) {
        self.cloneRange().collapse(false).insertNode(endNode).setEndBefore(
            endNode);
      }
  ...
  function _getRng(doc) {
    ...
    if (_IERANGE && (!rng || (!rng.item && rng.parentElement().ownerDocument
        !== doc))) {
      return null;
    }
    return rng;
  ...
  function _inPreElement(knode) {
    while (knode && knode.name != 'body') {
      if (_PRE_TAG_MAP[knode.name] || knode.name == 'div' && knode.hasClass(
          'ke-script')) {
        return true;
      }
      knode = knode.parent();
  ...
  _extend(KCmd, {
    ...
    select: function (hasDummy) {
      ...
      if (hasDummy && sc.nodeType == 1 && range.collapsed) {
        ...
        if (_WEBKIT) {
          ...
          if (K(sc).isInline() || so > 0 && K(children[so - 1]).isInline()
    ...
    commonAncestor: function (tagName) {
      var range = this.range,
          sc = range.startContainer, so = range.startOffset,
          ec = range.endContainer, eo = range.endOffset,
          startNode = (sc.nodeType == 3 || so === 0) ? sc : sc.childNodes[so
          - 1],
    ...
    fontfamily: function (val) {
      return this.wrap(
          '<span style="font-family:' + val + ';"></span>').select();
    },
      ...
      if (options.autoScroll) {
        if (_IE && _V < 7 || _QUIRKS) {
          ...
          K(self.win).bind('scroll', function (e) {
            ...
            self.pos(_removeUnit(self.x) + diffX, _removeUnit(self.y) + diffY,
                false);
          });
        } else {
  ...
  _extend(KEdit, KWidget, {
    init: function (options) {
      ...
      self.textarea = K(
          '<textarea class="ke-edit-textarea" hidefocus="true"></textarea>').css(
          'width', '100%');
      self.tabIndex = isNaN(parseInt(options.tabIndex, 10))
          ? self.srcElement.attr('tabindex') : parseInt(options.tabIndex, 10);
    ...
    html: function (val, isFull) {
      ...
      if (self.designMode) {
        ...
        if (val === undefined) {
          if (isFull) {
            val = '<!doctype html><html>' + body.parentNode.innerHTML
                + '</html>';
          } else {
            val = body.innerHTML;
          }
  ...
  _extend(KColorPicker, KWidget, {
    ...
    _addAttr: function (cell, color, cls) {
      ...
      if (color) {
        cell.append(
            K('<div class="ke-colorpicker-cell-color" unselectable="on"></div>').css(
  ...
  _extend(KUploadButton, {
    init: function (options) {
      ...
      for (var k in extraParams) {
        hiddenElements.push(
            '<input type="hidden" name="' + k + '" value="' + extraParams[k]
            + '" />');
      }
      var html = [
      ...
      self.div = div;
      self.button = button;
      self.iframe = options.target ? K('iframe[name="' + target + '"]') : K(
          'iframe', div);
      ...
      iframe.bind('load', function () {
        ...
        try {
          ...
          self.options.afterError.call(self,
              '<!doctype html><html>' + doc.body.parentNode.innerHTML
              + '</html>');
        }
        if (data) {
  ...
  function _createButton(arg) {
    arg = arg || {};
    var name = arg.name || '',
        span = K('<span class="ke-button-common ke-button-outer" title="' + name
            + '"></span>'),
        btn = K(
  ...
  _extend(KDialog, KWidget, {
    init: function (options) {
      ...
      if (_IE && _V < 7) {
        self.iframeMask = K(
            '<iframe src="about:blank" class="ke-dialog-shadow"></iframe>').appendTo(
            self.div);
      } else if (shadowMode) {
  ...
  KEditor.prototype = {
    ...
    create: function () {
      ...
      .addClass('ke-container ke-container-' + self.themeType).css('width',
          width);
      if (fullscreenMode) {
        container.css({
          position: 'absolute',
      ...
      K.each(self.items, function (i, name) {
        if (name == '|') {
          ...
          htmlList.push(
              '<span class="ke-outline" data-name="' + name + '" title="'
              + self.lang(name) + '" unselectable="on">');
          htmlList.push(
              '<span class="ke-toolbar-icon ke-toolbar-icon-url ke-icon-' + name
      ...
      var edit = self.edit = _edit({
        ...
        beforeGetHtml: function (html) {
          ...
          return _formatHtml(html, self.filterMode ? self.htmlTags : null,
              self.urlType, self.wellFormatMode, self.indentChar);
        },
    ...
    text: function (val) {
      ...
      if (val === undefined) {
        return _trim(
            self.html().replace(/<(?!img|embed).*?>/ig, '').replace(/&nbsp;/ig,
                ' '));
      } else {
    ...
    exec: function (key) {
      ...
      var self = this, cmd = self.cmd,
          changeFlag = _inArray(key, 'selectall,copy,paste,print'.split(','))
              < 0;
      if (changeFlag) {
  ...
  function _create(expr, options) {
    ...
    _loadScript(
        editor.langPath + editor.langType + '.js?ver=' + encodeURIComponent(
        K.DEBUG ? _TIME : _VERSION), function () {
          create(editor);
        });
    return editor;
  }
  ...
  _plugin('core', function (K) {
    ...
    self.clickToolbar('formatblock', function () {
      ...
      _each(blocks, function (key, val) {
        ...
        menu.addItem({
          title: '<span style="' + style + '" unselectable="on">' + val
              + '</span>',
    ...
    self.clickToolbar('fontname', function () {
      ...
      _each(self.lang('fontname.fontName'), function (key, val) {
        menu.addItem({
          title: '<span style="font-family: ' + key + ';" unselectable="on">'
              + val + '</span>',
          checked: (curVal === key.toLowerCase() || curVal
              === val.toLowerCase()),
          click: function () {
    ...
    self.clickToolbar('fontsize', function () {
      ...
      _each(self.fontSizeTable, function (i, val) {
        menu.addItem({
          title: '<span style="font-size:' + val + ';" unselectable="on">' + val
              + '</span>',
    ...
    _each(
        ...
        function (i, val) {
          var cond = _inArray(val, ['prop', 'delete']) < 0
              ? self.plugin.getSelectedCell : self.plugin.getSelectedTable;
          self.addContextmenu({
    ...
    self.beforeGetHtml(function (html) {
      ...
      .replace(
          ...
          function (full, attr, code) {
            return '<noscript' + unescape(attr) + '>' + unescape(code)
                + '</noscript>';
          })
    ...
    self.beforeSetHtml(function (html) {
      ...
      .replace(/<a[^>]*name="([^"]+)"[^>]*>(?:<\/a>)?/ig, function (full) {
        ...
        return '<img class="ke-anchor" src="' + self.themesPath
            + 'common/anchor.gif" data-ke-name="' + escape(attrs.name) + '" />';
      })
      .replace(/<script([^>]*)>([\s\S]*?)<\/script>/ig,
          function (full, attr, code) {
            return '<div class="ke-script" data-ke-script-attr="' + escape(attr)
                + '">' + escape(code) + '</div>';
          })
      .replace(/<noscript([^>]*)>([\s\S]*?)<\/noscript>/ig,
          function (full, attr, code) {
            return '<div class="ke-noscript" data-ke-noscript-attr="' + escape(
                attr) + '">' + escape(code) + '</div>';
          })
      .replace(/(<[^>]*)(href|src)="([^"]*)"([^>]*>)/ig,
          function (full, start, key, src, end) {
            ...
            full = start + key + '="' + src + '"' + ' data-ke-src="' + _escape(
                src) + '"' + end;
            return full;
          })
      .replace(/(<[^>]+\s)(on\w+="[^"]*"[^>]*>)/ig,
    ...
    edit: function () {
      ...
      var dialog = self.createDialog({
        ...
        yesBtn: {
          ...
          click: function (e) {
            self.insertHtml(
                '<a name="' + nameBox.val() + '">').hideDialog().focus();
          }
        }
      });
      var div = dialog.div,
...
KindEditor.plugin('baidumap', function (K) {
  ...
  self.clickToolbar(name, function () {
    var html = ['<div style="padding:10px 20px;">',
      ...
      '</div>',
      '<div class="ke-map" style="width:' + mapWidth + 'px;height:' + mapHeight
      + 'px;"></div>',
      '</div>'].join('');
    var dialog = self.createDialog({
      name: name,
      ...
      yesBtn: {
        ...
        click: function (e) {
          ...
          var url = [checkbox[0].checked ? self.pluginsPath
              + 'baidumap/index.html' : 'http://api.map.baidu.com/staticimage',
          ...
          if (checkbox[0].checked) {
            self.insertHtml(
                '<iframe src="' + url + '" frameborder="0" style="width:'
                + (mapWidth + 2) + 'px;height:' + (mapHeight + 2)
                + 'px;"></iframe>');
          } else {
    ...
    var div = dialog.div,
        ...
        win, doc;
    var iframe = K(
        '<iframe class="ke-textarea" frameborder="0" src="' + self.pluginsPath
        + 'baidumap/map.html" style="width:' + mapWidth + 'px;height:'
        + mapHeight + 'px;"></iframe>');
    ...
    function bindCellEvent(cell, j, num) {
      ...
      cell.click(function (e) {
        self.insertHtml('<img src="' + path + num
            + '.gif" border="0" alt="" />').hideMenu().focus();
        e.stop();
      });
    }
    ...
    function createList(result) {
      ...
      for (var i = 0, len = fileList.length; i < len; i++) {
        ...
        var iconUrl = imgPath + (data.is_dir ? 'folder-16.gif' : 'file-16.gif'),
            img = K('<img src="' + iconUrl + '" width="16" height="16" alt="'
                + data.filename + '" align="absmiddle" />'),
            cell0 = K(row[0].insertCell(0)).addClass('ke-cell ke-name').append(
                img).append(document.createTextNode(' ' + data.filename));
        ...
        K(row[0].insertCell(2)).addClass('ke-cell ke-datetime').html(
            data.datetime);
      }
    }
    ...
    function createView(result) {
      ...
      for (var i = 0, len = fileList.length; i < len; i++) {
        ...
        var fileUrl = result.current_url + data.filename,
            iconUrl = data.is_dir ? imgPath + 'folder-64.gif' : (data.is_photo
                ? fileUrl : imgPath + 'file-64.gif');
        var img = K('<img src="' + iconUrl + '" width="80" height="80" alt="'
            + data.filename + '" />');
        if (!data.is_dir || data.has_file) {
    ...
    var dialog = self.createDialog({
          ...
          yesBtn: {
            ...
            click: function (e) {
              ...
              // insert local image
              if (showLocal && showRemote && tabs && tabs.selectedIndex === 1
                  || !showRemote) {
      ...
      afterUpload: function (data) {
        ...
        if (data.error === 0) {
          ...
          if (!fillDescAfterUploadImage) {
            clickFn.call(self, url, data.title, data.width, data.height,
...
KindEditor.plugin('insertfile', function (K) {
  ...
  self.clickToolbar(name, function () {
    self.plugin.fileDialog({
      clickFn: function (url, title) {
        var html = '<a class="ke-insertfile" href="' + url + '" data-ke-src="'
  ...
  K.extend(KSWFUpload, {
    init: function (options) {
      ...
      self.div = K(options.container).html([
        ...
        '</div>',
        '<div class="ke-inline-block ke-swfupload-desc">' + options.uploadDesc
        + '</div>',
      ...
      function showError(itemDiv, msg) {
        ...
        K('.ke-message', itemDiv).addClass('ke-error').show().html(
            K.escape(msg));
      }
      ...
      var settings = {
        ...
        upload_success_handler: function (file, serverData) {
          ...
          file.url = data.url;
          K('.ke-img', itemDiv).attr('src', file.url).attr('data-status',
              file.filestatus).data('data', data);
          K('.ke-status > div', itemDiv).hide();
        }
      };
    ...
    appendFile: function (file) {
      var self = this;
      var itemDiv = K('<div class="ke-inline-block ke-item" data-id="' + file.id
          + '"></div>');
      ...
      var img = K('<img src="' + file.url + '" class="ke-img" data-status="'
  ...
  self.plugin.multiImageDialog = function (options) {
    ...
    var swfupload = K.swfupload({
      ...
      buttonImageUrl: imgPath + (self.langType == 'zh_CN'
          ? 'select-files-zh_CN.png' : 'select-files-en.png'),
  ...
  SWFUpload.prototype.initSettings = function () {
    this.ensureDefault = function (settingName, defaultValue) {
      this.settings[settingName] = (this.settings[settingName] == undefined)
          ? defaultValue : this.settings[settingName];
    };
      ...
      for (var name in postParams) {
        if (postParams.hasOwnProperty(name)) {
          paramStringPairs.push(
              encodeURIComponent(name.toString()) + "=" + encodeURIComponent(
  ...
  if (typeof (SWFUpload) === "function") {
    ...
    SWFUpload.queue.uploadStartHandler = function (file) {
      ...
      if (typeof (this.queueSettings.user_upload_start_handler)
          === "function") {
        returnValue = this.queueSettings.user_upload_start_handler.call(this,
            file);
      }
    ...
    SWFUpload.queue.uploadCompleteHandler = function (file) {
      ...
      if (typeof (user_upload_complete_handler) === "function") {
        continueUpload = (user_upload_complete_handler.call(this, file)
            === false) ? false : true;
      ...
      if (continueUpload) {
        var stats = this.getStats();
        if (stats.files_queued > 0 && this.queueSettings.queue_cancelled_flag
            === false) {
          this.startUpload();
  ...
  self.plugin.table = {
    ...
    prop: function (isInsert) {
      var html = [
        ...
        //background color
        '<div class="ke-dialog-row">',
        '<label for="keBgColor" style="width:90px;">' + lang.backgroundColor
        + '</label>',
        '<span class="ke-inline-block ke-input-color"></span>',
        '</div>',
        '</div>'
      ...
      if (table) {
        ...
        if (tableWidth !== undefined && (match = /^(\d+)((?:px|%)*)$/.exec(
            tableWidth))) {
        ...
        if (tableHeight !== undefined && (match = /^(\d+)((?:px|%)*)$/.exec(
            tableHeight))) {

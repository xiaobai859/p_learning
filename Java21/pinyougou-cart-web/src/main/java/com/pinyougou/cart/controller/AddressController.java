...
public class AddressController {
   ...
   */
  @RequestMapping("/add")
  public Result add(@RequestBody TbAddress addAddress) {

    String name = SecurityContextHolder.getContext().getAuthentication().getName();
    addAddress.setUserId(name);
    addAddress.setCreateDate(new Date());
    try {
  ...
  /**
   * @methodName:findListByLoginUser
   * @description: 根据登陆的用户名查询收货地址列表
   * @author：Xiaobai
   * @createTime：2019年8月13日 下午4:05:30
   * @remarks: @return
   * @resultType：List<TbAddress>
   */
  @RequestMapping("/findListByLoginUser")
  public List<TbAddress> findListByLoginUser() {
    String name = SecurityContextHolder.getContext().getAuthentication().getName();
    return addressService.fingAddressByUserId(name);
  }
